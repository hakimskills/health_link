...
class _AddProductPageState extends State<AddProductPage> {
              ...
              items: categories.map((String category) {
                return DropdownMenuItem<String>(
                  ...
                  child: Text(
                    category,
                    style: TextStyle(
                      color: category == "Other (Custom)" ? Color(0xFF008080) : Colors.black,
                      fontWeight: category == "Other (Custom)" ? FontWeight.w500 : FontWeight.normal,
                    ),
                  ),
                );
              }).toList(),
  ...
  List<Widget> getStepWidgets() => [
    ...
    Form(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 10),
          Text("Provide helpful details about the product, including specs and usage."),
          SizedBox(height: 15),
    ...
    Form(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Price & Stock", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          SizedBox(height: 10),
          ...
          if (widget.productType == 'inventory') ...[
            SizedBox(height: 10),
            _buildTextField(_inventoryPriceController, "Inventory Price", isNumeric: true),
          ],
          SizedBox(height: 10),
          _buildTextField(_stockController, "Stock Quantity", isNumeric: true),
        ],
      ),
    ),
    ...
    Form(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Product Images", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          SizedBox(height: 10),
          Text("Add clear photos to help users better understand the product. The first image will be set as the primary image."),
          SizedBox(height: 15),
          ...
          if (_selectedImages.isEmpty)
            Container(
              ...
              child: Column(
                ...
                children: [
                  Icon(Icons.image_outlined, color: Color(0xFF00C6A2), size: 40),
                  SizedBox(height: 8),
                  Text("No images selected",
                      style: TextStyle(color: Color(0xFF00C6A2), fontSize: 16, fontWeight: FontWeight.w500)),
                ],
              ),
            )
          else
                        ...
                        return Stack(
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                border: Border.all(
                                  color: index == 0 ? Colors.amber : Color(0xFF00C6A2),
                                  width: 2,
                                ),
                                borderRadius: BorderRadius.circular(10),
                              ),
  ...
  void _nextStep() {
    if (_formKeys[_currentStep].currentState!.validate()) {
      ...
      if (_currentStep == 3) {
        ...
        if (_showCustomCategory && _customCategoryController.text.trim().isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (_currentStep == 2 && widget.productType == 'inventory' &&
          (_inventoryPriceController.text.isEmpty || double.tryParse(_inventoryPriceController.text) == null)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Inventory price is required for inventory products')),
        );
        return;
      }
  ...
  Future<void> _submitProduct() async {
    ...
    if (widget.productType == 'inventory' && _inventoryPriceController.text.isNotEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    String pageTitle = widget.productType == 'new' ? "Add Store Product" : "Add Inventory Product";
