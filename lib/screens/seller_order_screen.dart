...
class _SellerOrdersScreenState extends State<SellerOrdersScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchSellerOrders() async {
    ...
    final url = 'http://192.168.43.101:8000/api/product-orders/seller/$sellerId';
  ...
  Future<void> approveOrder(int orderId) async {
    ...
    try {
      final response = await http.put(
        Uri.parse('http://192.168.43.101:8000/api/product-orders/$orderId/approve'),
        headers: {
          'Accept': 'application/json',
  ...
  List<dynamic> _getFilteredOrders(String status) {
    return _orders.where((order) {
      ...
      if (status == 'pending') {
        ...
        return orderStatus == 'delivered' || orderStatus == 'cancelled' || orderStatus == 'canceled';
      }
    }).toList();
  }
              ...
              child: Row(
                ...
                children: [
                  if (order['total_amount'] != null)
                    Text(
                      "Total: \$${order['total_amount'] is String ? order['total_amount'] : order['total_amount'].toStringAsFixed(2)}",
                      style: TextStyle(
                        color: primaryColor,
                        fontWeight: FontWeight.bold,
                        fontSize: 15,
...
class SellerOrderDetailsSheet extends StatelessWidget {
  ...
  Widget _buildOrderProgress(String status) {
    ...
    if (currentIndex == -1 || status.toLowerCase() == 'cancelled' || status.toLowerCase() == 'canceled') {
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              // Approval button for pending orders
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    // Approve button for pending orders (bottom)
                    if (isPending && onApprove != null)
                      ElevatedButton.icon(
                        onPressed: onApprove,
                        icon: const Icon(Icons.check_circle_outline),
